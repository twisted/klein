...
class KleinResource(Resource):
    ...
    @staticmethod
    def handle_match_exception(request, exception):
        """
        Handle any exception generated by mapper.match.
        Overriding this allows customising the display of exceptions like the HTTPException
        """
        if isinstance(exception, HTTPException):
            request.setResponseCode(exception.code)
            resp = exception.get_response({})

            for header, value in resp.headers:
                request.setHeader(header, value)
            return exception.get_body({})
        else:
            raise exception
    ...
    def render(self, request):
        ...
        try:
            ...
            return self.handle_match_exception(request, exception)
